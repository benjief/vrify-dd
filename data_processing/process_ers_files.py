...
# Process ERS files
def process_ers_files(folder, processed_folder, empty_ers_files_folder, ers_folder, total_ers_files, total_ers_files_skipped, total_size_kb):
        ...
        for file in files:
            if file.lower().endswith('.ers'):
                ...
                if is_ers_file_empty(ers_path):
                    total_ers_files_skipped += 1
                    total_size_kb = move_files(ers_path, ['.ERS', '', '.ERS.gi', '.ERS.aux.xml', '.ERS.xml'], empty_ers_files_folder, total_size_kb)
                    continue
                ...
                output_folder = os.path.join(ers_folder, crs_folder)
    ...
    return total_ers_files, total_size_kb